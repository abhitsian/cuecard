...
.leg b {{ font-size: 34px; font-weight: 800; min-width: 190px; display: inline-block; transform-origin: left center; }}
.leg .jev {{ color: #F7BA52; }} .leg .cl {{ color: #EDDBBD; }}
.leg span {{ color: #C9C5BE; }}
.boxes {{ display: flex; gap: 14px; margin-top: 6px; }}
.box {{ font-size: 24px; font-weight: 700; padding: 8px 16px; border-radius: 12px; display: inline-block; }}
...
// The title is fully on screen from the first frame (it is the thumbnail players and feeds show).
// Motion here only highlights: the four boxes pulse in turn, then the two engine lines.
tl.to("#t3 .box", {{scale: 1.08, duration: 0.25, ease: "power2.out", stagger: 0.3}}, 0.6);
tl.to("#t3 .box", {{scale: 1, duration: 0.3, ease: "power2.inOut", stagger: 0.3}}, 0.85);
tl.to("#t2 b", {{scale: 1.06, duration: 0.3, ease: "power2.out"}}, 2.0);
tl.to("#t2 b", {{scale: 1, duration: 0.3, ease: "power2.inOut"}}, 2.3);
tl.to("#t4 b", {{scale: 1.06, duration: 0.3, ease: "power2.out"}}, 2.8);
tl.to("#t4 b", {{scale: 1, duration: 0.3, ease: "power2.inOut"}}, 3.1);
